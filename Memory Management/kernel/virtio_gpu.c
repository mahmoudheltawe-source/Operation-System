...
static struct spinlock gpu_op_lock;
...
static uint64 orphaned_flip_pa[FB_PAGES];
static char orphaned_flip_owned[FB_PAGES];
static int orphaned_flip_valid;
static struct proc *flipped_proc;
static uint64 flipped_va;
static int flipped_valid;
...
static void gpu_transfer_flush(void);

static void
free_orphaned_flip(void)
{
    if (!orphaned_flip_valid)
        return;

    for (int i = 0; i < FB_PAGES; i++)
    {
        if (orphaned_flip_owned[i])
            kfree((void *)orphaned_flip_pa[i]);
        orphaned_flip_pa[i] = 0;
        orphaned_flip_owned[i] = 0;
    }

    orphaned_flip_valid = 0;
}

static int
is_kernel_fb_page(uint64 pa)
{
    for (int i = 0; i < FB_PAGES; i++)
        if (pa == (uint64)fb[i])
            return 1;
...
// Map the kernel-owned framebuffer pages into a user page table. These
// mappings are aliases of fb[]; callers must unmap them with do_free = 0.
int
virtio_gpu_map_display(pagetable_t pagetable, uint64 va)
{
    for (int i = 0; i < FB_PAGES; i++)
    {
        ...
        if (mappages(pagetable, page_va, PGSIZE, (uint64)fb[i],
                     PTE_U | PTE_R | PTE_W) < 0)
        {
            if (i > 0)
                uvmunmap(pagetable, va, i, 0);
...
void
virtio_gpu_unmap_display(pagetable_t pagetable, uint64 va)
{
...
virtio_gpu_flip(struct proc *p, uint64 va)
{
    ...
    for (int i = 0; i < FB_PAGES; i++)
    {
        uint64 pa = walkaddr(p->pagetable, va + (uint64)i * PGSIZE);
        if (pa == 0)
        {
            release(&gpu_op_lock);
            return -1;
...
void
virtio_gpu_release_process(struct proc *p)
{
    ...
    if (flipped_valid && flipped_proc == p)
    {
        int have_owned_pages = 0;

        for (int i = 0; i < FB_PAGES; i++)
        {
            pte_t *pte = walk(p->pagetable, flipped_va + (uint64)i * PGSIZE, 0);
            ...
            orphaned_flip_pa[i] = walkaddr(p->pagetable, flipped_va + (uint64)i * PGSIZE);
            if (is_kernel_fb_page(orphaned_flip_pa[i]))
            {
                orphaned_flip_owned[i] = 0;
            }
            else
            {
                orphaned_flip_owned[i] = 1;
                have_owned_pages = 1;
                *pte |= PTE_GPU;
            }
        }

        orphaned_flip_valid = have_owned_pages;
